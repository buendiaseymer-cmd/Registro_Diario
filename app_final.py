...
# ---------------------------------------------------------------------
# PESTAÑA 2: HOJA DE PRODUCCIÓN
# ---------------------------------------------------------------------
with tab2:
    st.markdown("<h3 style='text-align: center;'>Hoja de Producción</h3>", unsafe_allow_html=True)

    col1, col2, col3 = st.columns(3)
    with col1: fecha_prod = st.date_input("FECHA *", datetime.date.today(), key="fecha_prod")
    with col2: turno_prod = st.selectbox("TURNO *", ["DÍA", "NOCHE"], key="turno_prod")
    with col3: jefe_grupo_prod = st.text_input("JEFE DE GRUPO *", key="jefe_prod").upper()

    col4, col5 = st.columns(2)
    with col4: tramo_prod = st.text_input("TRAMO *", key="tramo_prod").upper()
    with col5: frente_prod = st.text_input("FRENTE DE TRABAJO *", key="frente_prod").upper()
    ...
    with st.expander("➕ Agregar personal no registrado"):
        ...
        with col_d: nuevo_dni = st.text_input("DNI Nuevo", key="tab2_dni")
        with col_n: nuevo_nombre = st.text_input("Nombre Nuevo", key="tab2_nombre").upper()
        with col_b:
            st.markdown("<br>", unsafe_allow_html=True)
    ...
    with st.form("form_produccion", clear_on_submit=True):
        ...
        df_actividades = st.data_editor(crear_tabla_actividades(), num_rows="dynamic",
                                        use_container_width=True, column_config=columnas_act)
        ...
        df_tareo = st.data_editor(crear_tabla_tareo(), num_rows="dynamic",
                                  use_container_width=True, column_config=columnas_tareo)
        ...
        df_equipos = st.data_editor(crear_tabla_equipos(), num_rows="dynamic",
                                    use_container_width=True, column_config=columnas_equipos)

        # --- BLOQUE 3.1: MATERIALES ---
        st.markdown("---")
        ...
        df_materiales = st.data_editor(crear_tabla_materiales(), num_rows="dynamic",
                                       use_container_width=True, column_config=columnas_materiales)
